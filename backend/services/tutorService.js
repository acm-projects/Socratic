...
const evaluatorPrompt = PromptTemplate.fromTemplate(`
  ...
  Score 3: Not all "why" are a 3 but if a user asks for help but provides context and asks "Why" like why we use a certain concept or User demonstrates critical thinking ("Why is the sample size cutoff 30 specifically?", "I'm trying to solve this problem, but I'm stuck, what am i missing?", "I know how to write a for loop, but I'm not sure how to use it to solve this problem, why do we use for loops how do they help our program?, Why does the CLT work even when the original distribution isn't normal?")
  Score 4: User connects the dots or asks for a practice problem ("Why is a hash map faster than an array for this specific type of data retrieval?", "so we use a for loop to iterate through each element of the array then?", "give me a practice problem over this topic")
  Score 5: Hypothetical "What if" scenarios that challenge assumptions or deep step-by-step reasoning ("What happens if my sample size is only 25 instead of 30?", "So, is it accurate to think of a variable like a physical box? If so, what happens if I try to put a box inside another box?", "I understand how this physics formula works for positive velocity, but what if the object is thrown backward? , "Why is the sample size cutoff 30 specifically? What happens if I use 25?)
  User Input: "{input}"
