    ...
    .then(data => {
      ...
      if (data.syllabus_url) {
        ...
        fetch('http://3.128.186.118:5000/api/syllabus/extract', {
          ...
          body: JSON.stringify({ 
            file_url: data.syllabus_url,
            class_code: courseId 
          })
        })
        .then(async res => {
          const text = await res.text()
          console.log("Extraction status:", res.status)
          console.log("Extraction response text:", text)
          try {
        ...
        .then(extractData => {
          console.log("Extraction parsed:", extractData)
          setTimeout(() => fetchSyllabusInfo(), 2000)
        })
  ...
  return (
    ...
    >
      ...
      <div className="p-9 flex flex-col flex-1 gap-7 min-h-0 overflow-hidden">
        ...
        <div className="bg-white/65 backdrop-blur-sm rounded-2xl px-12 py-4 h-fit shrink-0">
          <StudyHeatmap />
        </div>
      </div>
