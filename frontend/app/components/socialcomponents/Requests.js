...
function declineRequest(id) {
  setRequests(requests.filter(r => r.id !== id));
    ...
    return(
            <div className="bg-white w-[430px]] h-70 rounded-xl p-5 mt-5">
            ...
            <div className = "flex gap-2">
                ...
                onClick={() => acceptRequest(request.id)}
                  className="cursor-pointer bg-[#3D5C9B] hover:bg-[#3a5a8a] text-white text-xs font-medium px-4 py-1.5 rounded-full">
                  Accept
                </button>
                <button
...
{/*displays the modal to add friend*/}
        ...
        <div className="flex justify-end">
          ...
          className="cursor-pointer flex items-center gap-2 bg-[#3D5C9B] hover:bg-[#3a5a8a] text-white text-sm font-medium px-4 py-2 rounded-xl">
            Add Friend
          </button>
        </div>
      </div>
