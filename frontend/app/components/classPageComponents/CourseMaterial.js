          ...
          {files.map((file, i) => (
            <div key={i} className="flex items-center gap-3 py-4 border-b border-[#EAEEED] last:border-none">
              ...
              <div className="flex-1 min-w-0 pl-2">
               <p className="text-sm font-semibold text-[#141f1d] truncate">{file.file_name}</p>
               <p className="text-xs text-[#90aba7]">{file.uploaded_at ? new Date(file.uploaded_at).toLocaleDateString() : ""}</p>
              </div>
            </div>
          ))}
        </div>
