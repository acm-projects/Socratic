...
async function run() {
  try {
    ...
    const parser = StructuredOutputParser.fromNamesAndDescriptions({ //structured output parser to take structured output from LLM 
      score: "An integer from 0 to 5 reflecting the quality of the user input.", //score from 0 to 5
      reason: "A short, 1-sentence reason explaining why you assigned that score." //reason for the score
    });


    //evaluator prompt to score user input
    ...
    const evalChain = RunnableSequence.from([evaluatorPrompt, evaluatorLLM, parser]); //chains evaluator prompt with LLM reponse and parser to get structured format
...
Current question quality score: {score}/5 — {reason}.`
    ...
    while (true) {
      ...
      try {
        ...
        // Using raw search with score for deep debugging
        const resultsWithScores = await vectorStore.similaritySearchWithScore(fullQuery, 2);
        
        console.log(`\n[Vector Search] Query: "${fullQuery}"`); //query to get the relevant documents
        resultsWithScores.forEach(([doc, score], i) => { //loop through the relevant documents
          console.log(`  ${i+1}. Score: ${score.toFixed(4)} | ID: ${doc.metadata?.id || 'N/A'}`); 
          console.log(`     Text: "${doc.pageContent.substring(0, 100)}..."`); //print the relevant documents
        });
