...
async function test() {
  try {
    ...
    const eventData = {
      summary: "Test Google Calendar DisplayName",
      start: { dateTime: new Date(Date.now() + 3600*1000).toISOString() },
      end: { dateTime: new Date(Date.now() + 7200*1000).toISOString() },
      attendees: [
        { email: "testfakemail123321@example.com", displayName: "Fake Person Display Name" }
      ]
    };
    ...
    console.log("Returned Event Attendees:", JSON.stringify(created.data.attendees, null, 2));
