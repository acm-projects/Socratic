...
app.get("/users/:id/friends/achievements", async (req, res) => {
  try {
    ...
    friendsResult.rows.forEach(f => {
      friendMap[f.friend_id] = { first_name: f.first_name, last_name: f.last_name, image: f.image || null }
    })

    const achievementsResult = await pool.query(
      `SELECT ua.user_id, ua.achievement_id, ua.earned_at,
              a.name as achievement_name,
              a.icon_colored, a.icon_greyed
    ...
    const results = achievementsResult.rows.map(row => ({
      ...
      icon_colored: row.icon_colored || null,
      icon_greyed: row.icon_greyed || null,
